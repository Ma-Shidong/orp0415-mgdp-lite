...
// Yaw (rad) from /sim/odom. Used to align lidar sectorization with UAV heading.
// Keeping this in the raycaster avoids a major perception mismatch when the UAV yaw != 0.
double uav_yaw = 0.0;
...
void position_callback(const nav_msgs::Odometry::ConstPtr& pos_data) {
    ...
    // Extract yaw from quaternion (geometry_msgs uses x,y,z,w).
    const auto &q = pos_data->pose.pose.orientation;
...
void pcl_callback(const sensor_msgs::PointCloud2ConstPtr& msg) {
    ...
    *latest_cloud = *latest_cloud + *dynamic_cloud;
	...
	// Precompute yaw rotation (world -> body): rotate XY by -yaw.
	const float cy = static_cast<float>(std::cos(uav_yaw));
	const float sy = static_cast<float>(std::sin(uav_yaw));
    ...
    for(int i = 0; i < cloud_filtered->size(); i++) {
		...
		// Vector from UAV to point in world frame
		float dx_w = x - uav_position.x;
		float dy_w = y - uav_position.y;
		float dz = z - uav_position.z;
		// Rotate into UAV body frame (yaw-only)
		float dx = cy * dx_w + sy * dy_w;
		float dy = -sy * dx_w + cy * dy_w;
