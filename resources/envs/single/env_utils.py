...
def dobs_lidar_hits(lidar_range, dobs_height, cylinders, pos_w, ray_hits_dir,
                    error_tolerance=0.33, ray_chunk_size=2048, eps=1e-6):
    ...
    if ray_hits_dir.ndim == 4 and ray_hits_dir.shape[1] == 1:
        ray_hits_dir = ray_hits_dir.squeeze(1)

    cyl_pos = cylinders[:, 0, :2]
    cyl_radius = cylinders[:, 2, 0]

    pos_xy = pos_w[:, :2]
    d_horizontal = torch.norm(pos_xy[:, None, :] - cyl_pos[None, :, :], dim=-1)
    d_hit = d_horizontal - cyl_radius[None, :]

    n_envs, n_rays = ray_hits_dir.shape[:2]
    hits = torch.empty((n_envs, n_rays, 3), device=pos_w.device, dtype=pos_w.dtype)

    cyl_pos_exp = cyl_pos[None, :, None, :]
    cyl_r_exp = cyl_radius[None, :, None]
    pos_z = pos_w[:, 2]

    for start in range(0, n_rays, ray_chunk_size):
        end = min(n_rays, start + ray_chunk_size)
        ray_chunk = ray_hits_dir[:, start:end, :]
        ray_dir_xy = ray_chunk[:, :, :2]
        ray_dir_z = ray_chunk[:, :, 2]

        cos_theta = torch.norm(ray_dir_xy, dim=-1).clamp_min(eps)
        t = d_hit[:, :, None] / cos_theta[:, None, :]
        t = torch.where(t > 0, t, torch.full_like(t, lidar_range))

        z = pos_z[:, None, None] + t * ray_dir_z[:, None, :]
        valid_z = (z >= 0.0) & (z <= dobs_height)

        hit_xy = pos_xy[:, None, None, :] + t[..., None] * ray_dir_xy[:, None, :, :]
        xy_dist = torch.norm(hit_xy - cyl_pos_exp, dim=-1)
        valid_xy = xy_dist <= (cyl_r_exp + error_tolerance)

        valid = valid_z & valid_xy
        t_valid = torch.where(valid, t, torch.full_like(t, lidar_range))
        min_t = t_valid.min(dim=1).values

        hits[:, start:end, :] = pos_w[:, None, :] + min_t[..., None] * ray_chunk
