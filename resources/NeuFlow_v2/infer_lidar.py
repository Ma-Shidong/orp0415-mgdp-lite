...
def init_neuflow(bs, image_size, device=None):
    # device can be like "cuda:1" / torch.device(...)
    if device is None:
        device = os.environ.get("SIM_DEVICE", None) or ("cuda" if torch.cuda.is_available() else "cpu")
    ...
    # image_size is (H, W)
    model.init_bhwd(bs, image_size[0], image_size[1], str(device))
    return model
